...
# -*- coding: utf-8 -*-
"""
kino_bot.py — final, to'liq va tuzatilgan
Xususiyatlar:
- TTL tekshiruv (VALIDATION_TTL, default 3600s)
- Multi-line film meta, birinchi non-empty line = title (emoji bilan bo'lsa ham qabul qiladi)
- Admin panelga "Set Share Link" va "Remove Share Link"
- Har bir muvaffaqiyatli media yuborilgandan so'ng movies.downloads oshadi va caption yangilanadi
- DB migratsiyasi: agar eski DB bo'lsa, kerakli ustunlar qo'shiladi (movies.downloads, users.last_validated_at)
- Barcha yordamchi funksiyalar aniq va bir joyda e'lon qilingan
"""
...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8245774056:AAGOOCc6jAbTrS4AahPg88FPmKLMRL-CsDU")
if not BOT_TOKEN:
...
DB_FILE = os.getenv("DB_FILE", "kino_bot.db")
...
# ---------------- Helpers: parsing, urls, UI ----------------
def parse_chat_identifier(text: str) -> Optional[Dict[str, str]]:
    if not text:
        return None
    txt = text.strip()
    m = re.search(r"(?:https?://)?(?:www\.)?(?:t\.me|telegram\.me)/(.+)", txt, flags=re.I)
    if m:
        tail = m.group(1).strip()
        if tail.startswith("+") or tail.lower().startswith("joinchat/"):
            invite = txt if txt.lower().startswith("http") else "https://t.me/" + tail
            return {"type": "invite", "value": invite}
        username = re.split(r"[?\#]", tail)[0]
        if re.fullmatch(r"@?[A-Za-z0-9_]{3,}", username):
            if not username.startswith("@"):
                username = "@" + username
            return {"type": "username", "value": username}
    if txt.startswith("@") and re.fullmatch(r"@[A-Za-z0-9_]{3,}", txt):
        return {"type": "username", "value": txt}
    if re.fullmatch(r"-?\d{5,}", txt):
        return {"type": "id", "value": txt}
    if re.fullmatch(r"[A-Za-z0-9_]{3,}", txt):
        return {"type": "username", "value": "@" + txt}
    return None
...
async def groups_inline_kb(missing: List[Tuple[str, Optional[str]]]) -> InlineKeyboardMarkup:
    ...
    for cid, invite in missing:
        label, url = await resolve_display_for_inline(cid, invite)
        if url:
            rows.append([InlineKeyboardButton(text=label, url=url)])
        else:
            rows.append([InlineKeyboardButton(text=label, callback_data=f"dummy:{cid}")])
...
async def cmd_start(message: Message):
    ...
    if subscribed:
        await safe_send(message.from_user.id, "Siz avval tekshirilgansiz. Kino kodini yuboring; kod yuborilganda yangilangan tekshiruv amalga oshiriladi (agar kerak bo'lsa).")
        return

    intro = (
        "Assalomu alaykum! 🎬\n\n"
        "Kino kodini yuboring (masalan: 123). Kod yuborilganda obuna/join holatingiz avtomatik tekshiriladi.\n\n"
        "Agar siz guruh/kanalga a'zo bo'lmasangiz yoki join-request yuborgan bo'lsangiz — bot sizga qanday qilib a'zo bo'lish kerakligini ko'rsatadi."
    )
    await safe_send(message.from_user.id, intro)
...
async def admin_text_handler(message: Message):
    ...
    if st:
        ...
        if action == "add_group" and step == "wait_link":
            ident = text
            parsed = parse_chat_identifier(ident)
            if not parsed:
                admin_states.pop(ADMIN_ID, None)
        ...
        if action == "add_group" and step == "wait_chatid":
            ...
            parsed = parse_chat_identifier(chat_id_text)
            if parsed and parsed["type"] == "id":
                chat_id_to_save = parsed["value"]
            else:
                try:
        ...
        if action == "remove_group" and step == "wait_link":
            ...
            if not parsed:
                await safe_send(ADMIN_ID, "SSilka/identifierni tushunmadim.", reply_markup=admin_main_kb())
                return
            if parsed["type"] in ("id", "username"):
                try:
                    ch = await bot.get_chat(parsed["value"])
                    await remove_group_db(str(ch.id))
                    await safe_send(ADMIN_ID, f"Guruh {ch.id} dan olib tashlandi.", reply_markup=admin_main_kb())
                except Exception:
                    if parsed["type"] == "id":
                        await remove_group_db(parsed["value"])
                        await safe_send(ADMIN_ID, f"Guruh {parsed['value']} dan olib tashlandi.", reply_markup=admin_main_kb())
                    else:
                        await safe_send(ADMIN_ID, "Username resolve bo'lmadi; iltimos chat_id yuboring.", reply_markup=admin_main_kb())
            else:
                await remove_group_db(parsed["value"])
                await safe_send(ADMIN_ID, "Invite monitoring dan olib tashlandi.", reply_markup=admin_main_kb())
            return
        ...
        if action == "add_join" and step == "wait_chatid":
            ...
            try:
                try:
                    ch = await bot.get_chat(chat_id_to_save)
                    await add_join_monitored_db(str(ch.id), invite)
                    await safe_send(ADMIN_ID, f"JoinRequest monitoring qoʻshildi: chat_id={ch.id}, invite={invite or '-'}", reply_markup=admin_main_kb())
                except Exception:
                    await add_join_monitored_db(chat_id_to_save, invite)
                    await safe_send(ADMIN_ID, f"JoinRequest monitoring qoʻshildi (chat_id saqlandi): {chat_id_to_save}", reply_markup=admin_main_kb())
            except Exception as e:
        ...
        if action == "remove_join" and step == "wait_link":
            ...
            if parsed["type"] in ("id", "username"):
                try:
                    ...
                    if parsed["type"] == "id":
                        await remove_join_monitored_db(parsed["value"])
                        await safe_send(ADMIN_ID, f"JoinRequest monitoring {parsed['value']} dan olib tashlandi.", reply_markup=admin_main_kb())
                    else:
                        await safe_send(ADMIN_ID, "Username resolve bo'lmadi; iltimos chat_id yuboring.", reply_markup=admin_main_kb())
            else:
                await remove_join_monitored_db(parsed["value"])
        ...
        # Add Movie: wait_media -> wait_meta (multi-line meta allowed)
    ...
    # No active state -> admin commands from keyboard
    if text == "Add Group":
    ...
    if text == "Add JoinRequest":
        ...
        await safe_send(ADMIN_ID, "JoinRequest monitoring uchun ssilka yuboring (invite yoki @username yoki chat_id). Keyin chat_id so'raladi:", reply_markup=admin_flow_kb())
        return
...
async def on_chat_join_request(chat_join_request: ChatJoinRequest):
    try:
        ...
        monitored = await is_join_monitored_db(str(chat.id))
        if not monitored:
            monitored_list = await list_join_monitored_db()
            for stored_cid, inv in monitored_list:
                if inv and isinstance(inv, str) and (chat_join_request.invite_link and inv in chat_join_request.invite_link):
                    monitored = True
                    break
        if not monitored:
            logger.info("Join-request ignored for unmonitored chat %s", chat.id)
            return
